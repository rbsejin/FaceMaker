...
class TaskFileAdapter(
    ...
    class TaskFileViewHolder(
        ...
        val nameTextView = itemView.findViewById<TextView>(R.id.item_name_text)
        val deleteButton: ImageButton = itemView.findViewById(R.id.item_delete_button)
    }
    ...
    override fun onBindViewHolder(holder: TaskFileViewHolder, position: Int) {
        val taskFile = if (position == fileList.size) {
            ...
            holder.iconImageView.setImageResource(R.drawable.ic_photo_camera_24px)
            fileList[position]
        }
